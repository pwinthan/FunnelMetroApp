...
SELECT
  ...
  Stage,
  user_count,
  lag(user_count, 1, 0) OVER (ORDER BY funnel_step) AS previous_user_count,
  CASE
    WHEN lag(user_count, 1, 0) OVER (ORDER BY funnel_step) = 0 THEN 0 
    ELSE ROUND((1.0 - user_count::NUMERIC / lag(user_count, 1, 0) OVER (ORDER BY funnel_step)), 2)
  END AS user_drop_off_rate,
  ride_count,
  lag(ride_count, 1, 0) OVER (ORDER BY funnel_step) AS previous_ride_count,
  CASE
    WHEN lag(ride_count, 1, 0) OVER (ORDER BY funnel_step) = 0 THEN 0
    ELSE ROUND((1.0 - ride_count::NUMERIC / lag(ride_count, 1, 0) OVER (ORDER BY funnel_step)), 2)
